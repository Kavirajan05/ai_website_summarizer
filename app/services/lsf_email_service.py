...
def send_results_email(user_email: str, ai_data: dict):
    ...
    msg = EmailMessage()
    msg['Subject'] = f"Top {ai_data.get('service', 'Service').capitalize()} Providers in {ai_data.get('city', 'City').capitalize()}"
    ...
    for rec in ai_data.get('top_recommendations', []):
        ...
        # Add stars display
        try:
            stars = "⭐" * int(float(rating)) if rating != 'N/A' and float(rating) > 0 else ""
        except:
            stars = ""
    ...
    best_choice_maps = get_maps_link(best_choice.get('name', 'Best Choice'), best_choice.get('lat'), best_choice.get('lon'))
